...
class cmb_Meta_Box_field {
	...
	public function render_field() {
		...
		if ( 'title' == $this->type() || ! $this->args( 'show_names' ) || $is_side ) {
			...
			$tag   = 'th';
			printf( '<%1$s%2$s><label for="%3$s">%4$s</label><span class="description">%5$s</span></%1$s>', $tag, $style, $this->id(), $this->args( 'name' ), $this->args( 'subname' ));
	...
	public function _set_field_defaults( $args ) {
		...
		if ( ! isset( $args['subname'] ) ) $args['subname'] = '';
